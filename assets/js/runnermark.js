...
ApplicationMain.main = function() {
	...
	ApplicationMain.loaders.set("assets/RunnerMark.png",loader);
	ApplicationMain.total++;
	var urlLoader = new browser.net.URLLoader();
	urlLoader.dataFormat = browser.net.URLLoaderDataFormat.TEXT;
	ApplicationMain.urlLoaders.set("assets/runnermark.xml",urlLoader);
	ApplicationMain.total++;
	var loader1 = new browser.display.Loader();
	ApplicationMain.loaders.set("assets/scoreBg.png",loader1);
	ApplicationMain.total++;
...
ApplicationMain.preloader_onComplete = function(event) {
	...
	if(Reflect.field(RunnerMark,"main") == null) {
		var mainDisplayObj = new RunnerMark();
		if(js.Boot.__instanceof(mainDisplayObj,browser.display.DisplayObject)) nme.Lib.get_current().addChild(mainDisplayObj);
	} else Reflect.field(RunnerMark,"main").apply(RunnerMark,[]);
}
...
var RunnerMark = function() {
	browser.display.Sprite.call(this);
	haxe.Timer.delay($bind(this,this.init),250);
};
$hxClasses["RunnerMark"] = RunnerMark;
RunnerMark.__name__ = ["RunnerMark"];
RunnerMark.__super__ = browser.display.Sprite;
RunnerMark.prototype = $extend(browser.display.Sprite.prototype,{
	onRestartClicked: function(event) {
		this.get_stage().removeEventListener(browser.events.MouseEvent.CLICK,$bind(this,this.onRestartClicked));
		this.restartEngine();
	}
	,onEngineComplete: function() {
		while(this.nmeChildren.length > 0) this.removeChildAt(0);
		this.removeEventListener(browser.events.Event.ENTER_FRAME,$bind(this,this.onEnterFrame));
		var bg = new browser.display.Bitmap(nme.installer.Assets.getBitmapData("assets/scoreBg.png"));
		bg.set_x((this.stageWidth - bg.get_width()) / 2 | 0);
		bg.set_y((this.stageHeight - bg.get_height()) / 2 | 0);
		this.addChild(bg);
		var tf = new browser.text.TextFormat("_sans",48,16777215,true);
		var score = new browser.text.TextField();
		score.set_defaultTextFormat(tf);
		score.set_text("" + this.engine.get_runnerScore());
		score.set_width(300);
		score.set_height(50);
		score.set_x(bg.get_x() + (bg.get_width() - score.get_textWidth()) / 2 | 0);
		score.set_y(bg.get_y() + (bg.get_height() - score.get_textHeight()) / 2 | 0);
		this.addChild(score);
		this.get_stage().addEventListener(browser.events.MouseEvent.CLICK,$bind(this,this.onRestartClicked));
	}
	,createStats: function() {
		if(this.stats == null) this.stats = new utils.FPS(10,10,16777215);
		nme.Lib.get_current().addChild(this.stats);
	}
	...
	,onEnterFrame: function(event) {
		var elapsed = browser.Lib.getTimer() - this.prevTime;
		this.prevTime = browser.Lib.getTimer();
		if(this.engine != null) {
			this.engine.fps = this.stats.fps;
			this.engine.step(elapsed);
			this.stats.score = this.engine.get_runnerScore();
			this.layer.render();
		}
	}
	,createScene: function() {
		var tilesheet = new aze.display.SparrowTilesheet(nme.installer.Assets.getBitmapData("assets/RunnerMark.png"),this.sheetData);
		this.layer = new aze.display.TileLayer(tilesheet);
		ca.esdot.runnermark.RunnerEngine.targetFPS = 58;
		this.engine = new ca.esdot.runnermark.RunnerEngine(this.layer,this.stageWidth,this.stageHeight);
		this.engine.onComplete = $bind(this,this.onEngineComplete);
		this.addChild(this.engine);
		this.prevTime = browser.Lib.getTimer();
		this.createStats();
		this.addEventListener(browser.events.Event.ENTER_FRAME,$bind(this,this.onEnterFrame));
	}
	,ul_complete: function(e) {
		this.sheetData = e.target.data;
		this.createScene();
	}
	,init: function() {
		this.stageWidth = this.get_stage().get_stageWidth();
		this.stageHeight = this.get_stage().get_stageHeight();
		var ul = new browser.net.URLLoader(new browser.net.URLRequest("assets/RunnerMark.xml"));
		ul.addEventListener(browser.events.Event.COMPLETE,$bind(this,this.ul_complete));
	}
	,__class__: RunnerMark
});
var Std = function() { }
$hxClasses["Std"] = Std;
Std.__name__ = ["Std"];
...
Xml.prototype = {
	...
	,__class__: Xml
	,__properties__: {set_nodeName:"setNodeName",get_nodeName:"getNodeName",set_nodeValue:"setNodeValue",get_nodeValue:"getNodeValue"}
}
...
var aze = {}
aze.display = {}
aze.display.TilesheetEx = function(img) {
	browser.display.Tilesheet.call(this,img);
	this.defs = new Array();
	this.anims = new Hash();
	this.sizes = new Array();
};
$hxClasses["aze.display.TilesheetEx"] = aze.display.TilesheetEx;
aze.display.TilesheetEx.__name__ = ["aze","display","TilesheetEx"];
aze.display.TilesheetEx.__super__ = browser.display.Tilesheet;
aze.display.TilesheetEx.prototype = $extend(browser.display.Tilesheet.prototype,{
	getSize: function(indice) {
		if(indice < this.sizes.length) return this.sizes[indice]; else return new browser.geom.Rectangle();
	}
	,getAnim: function(name) {
		if(this.anims.exists(name)) return this.anims.get(name);
		var indices = new Array();
		var _g1 = 0, _g = this.defs.length;
		while(_g1 < _g) {
			var i = _g1++;
	...
	,__class__: aze.display.TilesheetEx
});
aze.display.SparrowTilesheet = function(img,xml) {
	aze.display.TilesheetEx.call(this,img);
	var ins = new browser.geom.Point(0,0);
	...
	while( $it0.hasNext() ) {
		...
		var rect = new browser.geom.Rectangle(Std.parseInt(texture.att.resolve("x")),Std.parseInt(texture.att.resolve("y")),Std.parseInt(texture.att.resolve("width")),Std.parseInt(texture.att.resolve("height")));
		var size = texture.has.resolve("frameX")?new browser.geom.Rectangle(Std.parseInt(texture.att.resolve("frameX")),Std.parseInt(texture.att.resolve("frameY")),Std.parseInt(texture.att.resolve("frameWidth")),Std.parseInt(texture.att.resolve("frameHeight"))):new browser.geom.Rectangle(0,0,rect.width,rect.height);
		var center = new browser.geom.Point(size.x + size.width / 2,size.y + size.height / 2);
		this.addDefinition(name,size,rect,center);
	}
};
$hxClasses["aze.display.SparrowTilesheet"] = aze.display.SparrowTilesheet;
aze.display.SparrowTilesheet.__name__ = ["aze","display","SparrowTilesheet"];
aze.display.SparrowTilesheet.__super__ = aze.display.TilesheetEx;
aze.display.SparrowTilesheet.prototype = $extend(aze.display.TilesheetEx.prototype,{
	__class__: aze.display.SparrowTilesheet
});
var haxe = {}
haxe.Public = function() { }
$hxClasses["haxe.Public"] = haxe.Public;
haxe.Public.__name__ = ["haxe","Public"];
aze.display.TileBase = function() {
	this.x = this.y = 0.0;
	this.visible = true;
};
$hxClasses["aze.display.TileBase"] = aze.display.TileBase;
aze.display.TileBase.__name__ = ["aze","display","TileBase"];
aze.display.TileBase.__interfaces__ = [haxe.Public];
aze.display.TileBase.prototype = {
	step: function(elapsed) {
	}
	,init: function(layer) {
		this.layer = layer;
	}
	,__class__: aze.display.TileBase
}
aze.display.TileSprite = function(tile) {
	aze.display.TileBase.call(this);
	this._rotation = 0;
	...
	this.set_tile(tile);
	this._indice = -1;
	this._transform = new Array();
};
$hxClasses["aze.display.TileSprite"] = aze.display.TileSprite;
aze.display.TileSprite.__name__ = ["aze","display","TileSprite"];
aze.display.TileSprite.__super__ = aze.display.TileBase;
aze.display.TileSprite.prototype = $extend(aze.display.TileBase.prototype,{
	get_width: function() {
	...
	,get_transform: function() {
		if(this.dirty == true) {
			this.dirty = false;
			var dirX = this._mirror == 1?-1:1;
			var dirY = this._mirror == 2?-1:1;
			if(this._rotation != 0) {
				var cos = Math.cos(-this._rotation);
				var sin = Math.sin(-this._rotation);
				this._transform[0] = dirX * cos * this._scaleX;
				this._transform[1] = dirY * sin * this._scaleY;
				this._transform[2] = -dirX * sin * this._scaleX;
				this._transform[3] = dirY * cos * this._scaleY;
			} else {
				this._transform[0] = dirX * this._scaleX;
				this._transform[1] = 0;
				this._transform[2] = 0;
				this._transform[3] = dirY * this._scaleY;
			}
		}
		return this._transform;
	}
	,set_color: function(value) {
		this.r = (value >> 16) / 255.0;
		this.g = (value >> 8 & 255) / 255.0;
		this.b = (value & 255) / 255.0;
		return value;
	}
	,get_color: function() {
		return (this.r * 256.0 | 0) << 16 + (this.g * 256.0 | 0) << 8 + (this.b * 256.0 | 0);
	}
	...
	,set_indice: function(value) {
		if(this._indice != value) this._indice = value;
		return value;
	}
	,set_tile: function(value) {
		if(this._tile != value) {
			this._tile = value;
			if(this.layer != null) this.init(this.layer);
		}
		return value;
	}
	,init: function(layer) {
		this.layer = layer;
		var indices = layer.tilesheet.getAnim(this._tile);
		this.set_indice(indices[0]);
		this.size = layer.tilesheet.getSize(this._indice);
	}
	,__class__: aze.display.TileSprite
	,__properties__: {set_tile:"set_tile",get_tile:"get_tile",set_indice:"set_indice",get_indice:"get_indice",set_mirror:"set_mirror",get_mirror:"get_mirror",set_rotation:"set_rotation",get_rotation:"get_rotation",set_scale:"set_scale",get_scale:"get_scale",set_scaleX:"set_scaleX",get_scaleX:"get_scaleX",set_scaleY:"set_scaleY",get_scaleY:"get_scaleY",get_transform:"get_transform",get_height:"get_height",get_width:"get_width"}
});
aze.display.TileClip = function(tile,fps) {
	if(fps == null) fps = 18;
	aze.display.TileSprite.call(this,tile);
	this.fps = fps;
	this.animated = this.loop = true;
	this.time = 0;
	this.prevFrame = -1;
};
$hxClasses["aze.display.TileClip"] = aze.display.TileClip;
aze.display.TileClip.__name__ = ["aze","display","TileClip"];
aze.display.TileClip.__super__ = aze.display.TileSprite;
aze.display.TileClip.prototype = $extend(aze.display.TileSprite.prototype,{
	set_currentFrame: function(value) {
		if(value >= this.frames.length) value = this.frames.length - 1;
		this.time = Math.floor(1000 * value / this.fps) + 1;
		this.set_indice(this.frames[value]);
		return value;
	}
	...
	,step: function(elapsed) {
		...
		if(looping) {
			if(!this.loop) {
				this.animated = false;
				this.set_currentFrame(this.frames.length - 1);
			} else this.set_indice(this.frames[newFrame]);
	...
	,__class__: aze.display.TileClip
	,__properties__: $extend(aze.display.TileSprite.prototype.__properties__,{set_currentFrame:"set_currentFrame",get_currentFrame:"get_currentFrame",get_totalFrames:"get_totalFrames"})
});
aze.display.TileGroup = function() {
	aze.display.TileBase.call(this);
	this.children = new Array();
};
$hxClasses["aze.display.TileGroup"] = aze.display.TileGroup;
aze.display.TileGroup.__name__ = ["aze","display","TileGroup"];
aze.display.TileGroup.__super__ = aze.display.TileBase;
aze.display.TileGroup.prototype = $extend(aze.display.TileBase.prototype,{
	get_width: function() {
		if((this.children != null?this.children.length:0) == 0) return 0;
		var xmin = 9999.0, xmax = -9999.0;
		var _g = 0, _g1 = this.children;
		while(_g < _g1.length) {
			var child = _g1[_g];
			++_g;
			if(js.Boot.__instanceof(child,aze.display.TileSprite)) {
				var sprite = child;
				var w = sprite.size.width * sprite._scaleX;
	...
	,get_height: function() {
		...
		while(_g < _g1.length) {
			...
			if(js.Boot.__instanceof(child,aze.display.TileSprite)) {
				var sprite = child;
				var h = sprite.size.height * sprite._scaleY;
	...
	,removeChild: function(item) {
		...
		var index = Lambda.indexOf(this.children,item);
		if(index >= 0) {
	...
	,initChild: function(item) {
		item.parent = this;
		if(this.layer != null && item.layer == null) item.init(this.layer);
	}
	,init: function(layer) {
		this.layer = layer;
		this.initChildren();
	}
	,__class__: aze.display.TileGroup
	,__properties__: {get_numChildren:"get_numChildren",get_height:"get_height",get_width:"get_width"}
});
aze.display.TileLayer = function(tilesheet,smooth,additive) {
	if(additive == null) additive = false;
	if(smooth == null) smooth = true;
	aze.display.TileGroup.call(this);
	this.view = new browser.display.Sprite();
	...
	this.init(this);
	this.drawList = new aze.display.DrawList();
};
$hxClasses["aze.display.TileLayer"] = aze.display.TileLayer;
aze.display.TileLayer.__name__ = ["aze","display","TileLayer"];
aze.display.TileLayer.__super__ = aze.display.TileGroup;
aze.display.TileLayer.prototype = $extend(aze.display.TileGroup.prototype,{
	renderGroup: function(group,index,gx,gy) {
		var list = this.drawList.list;
		var fields = this.drawList.fields;
		var offsetTransform = this.drawList.offsetTransform;
		var offsetRGB = this.drawList.offsetRGB;
		var offsetAlpha = this.drawList.offsetAlpha;
		var elapsed = this.drawList.elapsed;
		gx += group.x;
		gy += group.y;
		var n = group.children != null?group.children.length:0;
		var _g = 0;
		while(_g < n) {
			var i = _g++;
			var child = group.children[i];
			if(child.animated) child.step(elapsed);
			var group1 = js.Boot.__instanceof(child,aze.display.TileGroup)?child:null;
			if(group1 != null) index = this.renderGroup(group1,index,gx,gy); else {
				var sprite = child;
				if(sprite.alpha <= 0.0) continue;
				list[index] = sprite.x + gx;
				list[index + 1] = sprite.y + gy;
				list[index + 2] = sprite._indice;
				if(offsetTransform > 0) {
					var t = sprite.get_transform();
	...
	,render: function(elapsed) {
		this.drawList.begin(elapsed == null?0:elapsed,this.useTransforms,this.useAlpha,this.useTint,this.useAdditive);
	...
	,__class__: aze.display.TileLayer
});
aze.display.DrawList = function() {
	this.list = new Array();
	this.elapsed = 0;
	this.runs = 0;
};
$hxClasses["aze.display.DrawList"] = aze.display.DrawList;
aze.display.DrawList.__name__ = ["aze","display","DrawList"];
aze.display.DrawList.__interfaces__ = [haxe.Public];
aze.display.DrawList.prototype = {
	end: function() {
	...
	,begin: function(elapsed,useTransforms,useAlpha,useTint,useAdditive) {
		...
		if(elapsed > 0) this.elapsed = elapsed; else {
			...
			if(this.time > 0) {
				...
				this.time = t;
			} else this.time = browser.Lib.getTimer();
		}
	}
	,__class__: aze.display.DrawList
}
...
ca.esdot = {}
ca.esdot.runnermark = {}
ca.esdot.runnermark.RunnerEngine = function(layer,stageWidth,stageHeight) {
	browser.display.Sprite.call(this);
	this.layer = layer;
	this._root = layer;
	this.lastIncrement = browser.Lib.getTimer() + 2000;
	this.fps = -1;
	this.steps = 0;
	this.set_runnerScore(0);
	this.incrementDelay = 250;
	this.maxIncrement = 12000;
	this.spritePool = new Hash();
	this.tilePool = new Hash();
	...
	this.sky.set_width(stageWidth);
	this.sky.set_height(stageHeight);
	this.bgStrip1.y = stageHeight - (this.bgStrip1.get_height() / 2 | 0) - 50;
	this.bgStrip2.y = stageHeight - (this.bgStrip2.get_height() / 2 | 0) - 50;
	this.groundY = stageHeight - 50 | 0;
	this.runner.x = stageWidth * .2;
	this.runner.y = this.groundY - 65;
	this.runner.groundY = this.runner.y | 0;
	this.runner.enemyList = this.enemyList;
	this.addGround(3);
	this.addParticles(32);
	this.addChild(layer.view);
};
$hxClasses["ca.esdot.runnermark.RunnerEngine"] = ca.esdot.runnermark.RunnerEngine;
ca.esdot.runnermark.RunnerEngine.__name__ = ["ca","esdot","runnermark","RunnerEngine"];
ca.esdot.runnermark.RunnerEngine.__super__ = browser.display.Sprite;
ca.esdot.runnermark.RunnerEngine.prototype = $extend(browser.display.Sprite.prototype,{
	putTile: function(sprite) {
		if(sprite.parent != null) sprite.parent.removeChild(sprite);
		sprite.x = sprite.y = 0;
		sprite.set_scale(1);
		sprite.alpha = 1;
		sprite.set_rotation(0);
		if(!this.tilePool.exists(sprite._tile)) this.tilePool.set(sprite._tile,new Array());
		this.tilePool.get(sprite._tile).push(sprite);
	}
	,getTile: function(type) {
		if(this.tilePool.exists(type)) return this.tilePool.get(type).pop();
		return null;
	}
	,getSprite: function(type) {
		if(this.spritePool.exists(type)) return this.spritePool.get(type).pop();
		return null;
	}
	,get_numEnemies: function() {
		return this.enemyList.length;
	}
	,createSkyData: function() {
		var m = new browser.geom.Matrix();
		m.createGradientBox(64,64,Math.PI / 2);
		var rect = new browser.display.Sprite();
		rect.get_graphics().beginGradientFill(browser.display.GradientType.LINEAR,[0,1968478],[1,.5],[0,255],m);
		rect.get_graphics().drawRect(0,0,128,128);
		var col = 0;
		var skyData = new browser.display.BitmapData(128,128,false,col);
		skyData.draw(rect);
		return skyData;
	}
	,addEnemies: function(numEnemies) {
		if(numEnemies == null) numEnemies = 1;
		var enemy;
		var _g = 0;
		while(_g < numEnemies) {
			var i = _g++;
			enemy = this.createEnemy();
			enemy.y = this.groundY - enemy.size.height * enemy._scaleY / 2 + 12;
			enemy.x = this.stageWidth - 50 + Math.random() * 100;
			enemy.groundY = enemy.y | 0;
			enemy.y = -(enemy.size.height * enemy._scaleY);
			this.enemyList.push(enemy);
		}
	}
	,addParticles: function(numParticles) {
		var p;
		var _g = 0;
		while(_g < numParticles) {
			var i = _g++;
			p = this.createParticle();
			p.x = this.runner.x - 40;
			p.y = this.runner.y + this.runner.get_height() / 4 + this.runner.get_height() * .25 * Math.random() - 10;
			this.particleList.push(p);
			p.set_scale(0.6);
			p.alpha = 0.6;
		}
	}
	,addGround: function(numPieces,height) {
		if(height == null) height = 0;
		var lastX = 0;
		if(this.lastGroundPiece != null) lastX = this.lastGroundPiece.x + this.lastGroundPiece.get_width() / 2 - 1 | 0;
		var piece = null;
		var _g = 0;
		while(_g < numPieces) {
			var i = _g++;
			piece = this.createGroundPiece();
			piece.y = this.groundY + piece.size.height * piece._scaleY / 2 - height;
			piece.x = lastX + piece.size.width * piece._scaleX / 2;
			lastX += piece.size.width * piece._scaleX / 2 - 1 | 0;
			this.groundList.push(piece);
		}
		if(height == 0) this.lastGroundPiece = piece;
	}
	,stopEngine: function() {
		while(this._root.get_numChildren() > 0) this._root.removeChildAt(0);
	}
	,step: function(elapsed) {
		this.steps++;
		if(this.enemyList.length > 3) this.set_runnerScore(ca.esdot.runnermark.RunnerEngine.targetFPS * 10 + this.enemyList.length); else if(this.fps > 0) this.set_runnerScore(this.fps * 10);
		var _g = this.runner;
		_g.set_rotation(_g._rotation + 0.1);
		this.updateRunner(elapsed);
		this.updateBg(elapsed);
		if(this.enemyList != null) this.updateEnemies(elapsed);
		if(this.groundList != null) this.updateGround(elapsed);
		this.updateParticles(elapsed);
		var increment = browser.Lib.getTimer() - this.lastIncrement;
		if(this.fps >= ca.esdot.runnermark.RunnerEngine.targetFPS && increment > this.incrementDelay) {
			this.addEnemies(1 + Math.floor(this.enemyList.length / 50) | 0);
			this.lastIncrement = browser.Lib.getTimer();
		} else if(this.fps < ca.esdot.runnermark.RunnerEngine.targetFPS && this.enemyList.length < 3 && increment > this.incrementDelay) {
			this.addEnemies(1);
			this.incrementDelay = increment + 500;
			this.lastIncrement = browser.Lib.getTimer();
		} else if(increment > this.maxIncrement) {
			if(this.onComplete != null) this.onComplete();
			this.stopEngine();
		}
	}
	,updateEnemies: function(elapsed) {
		var enemy;
		var i = this.enemyList.length - 1;
		while(i >= 0) {
			enemy = this.enemyList[i];
			enemy.x -= elapsed * .33;
			enemy.update();
			if(enemy.x < -(enemy.size.width * enemy._scaleX)) enemy.x = this.stageWidth + 20;
			i--;
		}
	}
	,updateParticles: function(elapsed) {
		if(this.steps % 3 == 0) this.addParticles(3);
		var p;
		var i = this.particleList.length - 1;
		while(i >= 0) {
			p = this.particleList[i];
			p.x -= elapsed * 0.33 * .2;
			p.alpha -= .01;
			var _g = p;
			_g.set_scale(_g._scaleX - .01);
			if(p.alpha < 0 || p._scaleX < 0) {
	...
	,updateGround: function(elapsed) {
		if(this.steps % (this.fps > 30?100:50) == 0) this.addGround(1,this.stageHeight * .25 + this.stageHeight * .5 * Math.random() | 0);
		var ground;
		var i = this.groundList.length - 1;
		while(i >= 0) {
			ground = this.groundList[i];
			ground.x -= elapsed * 0.33;
			if(ground.x < -(ground.size.width * ground._scaleX) / 2) {
				this.groundList.splice(i,1);
				this.putTile(ground);
			}
			i--;
		}
		var lastX = this.lastGroundPiece != null?this.lastGroundPiece.x + this.lastGroundPiece.get_width() / 2:0;
		if(lastX < this.stageWidth) this.addGround(1,0);
	}
	,updateBg: function(elapsed) {
		this.bgStrip1.x -= elapsed * 0.33 * .25;
		if(this.bgStrip1.x < -this.bgStrip1.get_width() / 2) this.bgStrip1.x = 0;
		this.bgStrip2.x -= elapsed * 0.33 * .5;
		if(this.bgStrip2.x < -this.bgStrip2.get_width() / 2) this.bgStrip2.x = 0;
	}
	,updateRunner: function(elapsed) {
		this.runner.update();
	}
	,createEnemy: function() {
		var sprite = this.getSprite("Enemy");
		if(sprite == null) sprite = new ca.esdot.runnermark.sprites.EnemySprite("Enemy");
		sprite.set_scale(0.6 + 0.4 * Math.random());
		sprite.set_mirror(Math.random() > 0.5?1:0);
		this._root.addChildAt(sprite,this._root.getChildIndex(this.runner) - 1);
		return sprite;
	}
	,createParticle: function() {
		var sprite = this.getTile("cloud");
		if(sprite == null) sprite = new aze.display.TileSprite("cloud");
		this._root.addChild(sprite);
		return sprite;
	}
	,createGroundPiece: function() {
		var sprite = this.getTile("ground");
		if(sprite == null) sprite = new aze.display.TileSprite("ground");
		this._root.addChildAt(sprite,this._root.getChildIndex(this.bgStrip2) + 1);
		return sprite;
	}
	,createChildren: function() {
		var skyData = this.createSkyData();
		this.sky = new browser.display.Bitmap(skyData);
		this.addChild(this.sky);
		var bitmap1, bitmap2;
		this.bgStrip1 = new aze.display.TileGroup();
		bitmap1 = new aze.display.TileSprite("bg1");
		bitmap1.set_scale(2);
		this.bgStrip1.addChild(bitmap1);
		bitmap2 = new aze.display.TileSprite("bg1");
		bitmap2.set_scale(2);
		this.bgStrip1.addChild(bitmap2);
		this._root.addChild(this.bgStrip1);
		bitmap1.x = bitmap1.size.width * bitmap1._scaleX / 2;
		bitmap2.x = bitmap1.x + bitmap1.size.width * bitmap1._scaleX;
		this.bgStrip2 = new aze.display.TileGroup();
		bitmap1 = new aze.display.TileSprite("bg2");
		bitmap1.set_scale(2);
		this.bgStrip2.addChild(bitmap1);
		bitmap2 = new aze.display.TileSprite("bg2");
		bitmap2.set_scale(2);
		this.bgStrip2.addChild(bitmap2);
		this._root.addChild(this.bgStrip2);
		bitmap1.x = bitmap1.size.width * bitmap1._scaleX / 2;
		bitmap2.x = bitmap1.x + bitmap1.size.width * bitmap1._scaleX;
		this.runner = new ca.esdot.runnermark.sprites.RunnerSprite("Runner");
		this._root.addChild(this.runner);
	}
	,set_runnerScore: function(value) {
		this._runnerScore = value;
		return this._runnerScore;
	}
	,get_runnerScore: function() {
		return this._runnerScore;
	}
	,__class__: ca.esdot.runnermark.RunnerEngine
	,__properties__: $extend(browser.display.Sprite.prototype.__properties__,{set_runnerScore:"set_runnerScore",get_runnerScore:"get_runnerScore"})
});
ca.esdot.runnermark.sprites = {}
ca.esdot.runnermark.sprites.GenericSprite = function(type) {
	aze.display.TileClip.call(this,type);
	this.gravity = 1;
	this.velY = 0;
};
$hxClasses["ca.esdot.runnermark.sprites.GenericSprite"] = ca.esdot.runnermark.sprites.GenericSprite;
ca.esdot.runnermark.sprites.GenericSprite.__name__ = ["ca","esdot","runnermark","sprites","GenericSprite"];
ca.esdot.runnermark.sprites.GenericSprite.__super__ = aze.display.TileClip;
ca.esdot.runnermark.sprites.GenericSprite.prototype = $extend(aze.display.TileClip.prototype,{
	__class__: ca.esdot.runnermark.sprites.GenericSprite
});
ca.esdot.runnermark.sprites.EnemySprite = function(type) {
	ca.esdot.runnermark.sprites.GenericSprite.call(this,type);
};
$hxClasses["ca.esdot.runnermark.sprites.EnemySprite"] = ca.esdot.runnermark.sprites.EnemySprite;
ca.esdot.runnermark.sprites.EnemySprite.__name__ = ["ca","esdot","runnermark","sprites","EnemySprite"];
ca.esdot.runnermark.sprites.EnemySprite.__super__ = ca.esdot.runnermark.sprites.GenericSprite;
ca.esdot.runnermark.sprites.EnemySprite.prototype = $extend(ca.esdot.runnermark.sprites.GenericSprite.prototype,{
	update: function() {
		...
		if(!this.isJumping && this.y == this.groundY && Math.random() < .02) {
			this.velY = -(this.size.height * this._scaleY) * .25;
			this.isJumping = true;
		}
	}
	,__class__: ca.esdot.runnermark.sprites.EnemySprite
});
ca.esdot.runnermark.sprites.RunnerSprite = function(type) {
	ca.esdot.runnermark.sprites.GenericSprite.call(this,type);
};
$hxClasses["ca.esdot.runnermark.sprites.RunnerSprite"] = ca.esdot.runnermark.sprites.RunnerSprite;
ca.esdot.runnermark.sprites.RunnerSprite.__name__ = ["ca","esdot","runnermark","sprites","RunnerSprite"];
ca.esdot.runnermark.sprites.RunnerSprite.__super__ = ca.esdot.runnermark.sprites.GenericSprite;
ca.esdot.runnermark.sprites.RunnerSprite.prototype = $extend(ca.esdot.runnermark.sprites.GenericSprite.prototype,{
	update: function() {
		this.velY += this.gravity;
		this.y += this.velY;
		if(this.y > this.groundY) {
		...
		if(this.enemyList == null || this.isJumping) return;
		var w = this.size.width * this._scaleX;
		var _g = 0, _g1 = this.enemyList;
		while(_g < _g1.length) {
			var enemy = _g1[_g];
			++_g;
			if(enemy.x > this.x && enemy.x - this.x < w * 1.5) {
				this.velY = -22;
				this.isJumping = true;
				break;
			}
	...
	,__class__: ca.esdot.runnermark.sprites.RunnerSprite
});
...
nme.installer.Assets.initialize = function() {
	if(!nme.installer.Assets.initialized) {
		nme.installer.Assets.resourceNames.set("assets/RunnerMark.png","assets/RunnerMark.png");
		nme.installer.Assets.resourceTypes.set("assets/RunnerMark.png","image");
		nme.installer.Assets.resourceNames.set("assets/RunnerMark.xml","assets/RunnerMark.xml");
		nme.installer.Assets.resourceTypes.set("assets/RunnerMark.xml","text");
		nme.installer.Assets.resourceNames.set("assets/scoreBg.png","assets/scoreBg.png");
		nme.installer.Assets.resourceTypes.set("assets/scoreBg.png","image");
...
var utils = {}
utils.FPS = function(inX,inY,inCol) {
	if(inCol == null) inCol = 0;
	if(inY == null) inY = 10.0;
	if(inX == null) inX = 10.0;
	browser.text.TextField.call(this);
	this.set_x(inX);
	this.set_y(inY);
	this.selectable = false;
	this.set_defaultTextFormat(new browser.text.TextFormat("_sans",20,inCol,true));
	this.set_htmlText("FPS:");
	this.multiline = true;
	this.set_width(150);
	this.times = [];
	this.addEventListener(browser.events.Event.ENTER_FRAME,$bind(this,this.onEnter));
};
$hxClasses["utils.FPS"] = utils.FPS;
utils.FPS.__name__ = ["utils","FPS"];
utils.FPS.__super__ = browser.text.TextField;
utils.FPS.prototype = $extend(browser.text.TextField.prototype,{
	onEnter: function(_) {
		var now = browser.Lib.getTimer() / 1000;
		this.times.push(now);
		while(this.times[0] < now - 1) this.times.shift();
		this.fps = this.times.length;
		if(this.get_visible() && (this.prev != this.fps || this.prevScore != this.score)) {
			this.prev = this.fps;
			this.prevScore = this.score;
			this.set_htmlText("FPS: " + this.fps + "<br/>score: " + this.score);
		}
	}
	,__class__: utils.FPS
});
...
//@ sourceMappingURL=RunnerMark.js.map